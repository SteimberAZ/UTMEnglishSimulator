...
<body>
    ...
    <script>
        ...
        // Algoritmo de distancia Levenshtein ultrarrápido para Fuzzy Matching (tolerancia a errores de OCR)
        ...
        // Pre-tokenizar palabras clave en memoria para búsquedas difusas instantáneas (<2ms)
        DB.forEach(item => {
            const wordsList = item.k.match(/[a-z]{4,}/g) || [];
            item.w = Array.from(new Set(wordsList.filter(w => !STOP_WORDS.has(w))));
        });

        function search() {
            let rawQ = document.getElementById('queryInput').value.trim().toLowerCase();
            ...
            // Normalizar errores clásicos de escaneo OCR
            rawQ = rawQ
                .replace(/\\|/g, 'l')
            ...
            for (let item of DB) {
                ...
                // 2. Bonus exacto de n-gramas
                for (let ng of ngrams) {
                    if (qLower.includes(ng)) score += 25;
                    else if (kLower.includes(ng)) score += 12;
                }

                // 3. Puntuación por términos (Exacto + Fuzzy Matching con Levenshtein)
                ...
                for (let w of words) {
                    ...
                    // Coincidencia difusa para errores tipográficos del OCR
                    if (!hit && !isStop && w.length >= 4) {
                        const maxDist = w.length <= 5 ? 1 : 2;
                        for (let tw of item.w) {
                            if (Math.abs(tw.length - w.length) <= maxDist) {
